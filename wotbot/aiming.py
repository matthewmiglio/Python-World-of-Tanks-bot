...
def amt_to_aim_calculator(pix_to_move):
    # takes in pixels to move and returns a duration in seconds to use for
    # turning camera
...
def draw_picture_for_aiming(coords, region):
    ...
    black_image = Image.open(
        r"C:\Users\Matt\Desktop\1\my Programs\wot-bot\reference_images\draw_images\black4.png")
...
def find_enemy():
    ...
    # doctor the photo to cover the minimap and the reload timer (which is
    # sometimes red and may interfere with some detection idrk)
    reload_timer_cover_image = Image.open(
        r"C:\Users\Matt\Desktop\1\my Programs\wot-bot\reference_images\draw_images\black5.png")
    minimap_cover_image = Image.open(
        r"C:\Users\Matt\Desktop\1\my Programs\wot-bot\reference_images\draw_images\black6.png")
    enemy_player_count_image = Image.open(
        r"C:\Users\Matt\Desktop\1\my Programs\wot-bot\reference_images\draw_images\black7.png")
    ...
    # using the coord list, return back any pixels that have a certain amount
    # of other pixels around them.
    enemy_coords_list = find_enemy_given_red_image(coords)

    # using the next coord list, group up the coords and return consolidated
    # estimated coordinates of enemy players.
...
def find_enemy_given_red_image(coord_list):
    ...
    while index_1 > -1:
        ...
        while index_2 > -1:
            ...
            if coords_is_equal(coords_1, coords_2, tol=45):
                pix_in_radius = pix_in_radius + 1

            # check the counter. if its above whatever then its a positive case
            # so we add to list of positive case coords
            if pix_in_radius > 15:
    ...
    # if we exit the loop without ever triggering the counter case we return
    # None
...
def find_enemy_coord_closest_to_crosshair(coord_list):
    ...
    while index > -1:
        ...
        total_difference_value = (
            current_x_difference * current_x_difference) + (
            current_y_difference * current_y_difference)
        if (min_total_difference_value is None) or (
                min_total_difference_coords is None):
...
def find_red_pix_in_image(ss):
    ...
    while x_coord < width:
        ...
        while y_coord < height:
            ...
            if pixel_is_equal(sent1, pix, tol=15):
                is_red = True
            if pixel_is_equal(sent2, pix, tol=15):
                is_red = True
            if pixel_is_equal(sent3, pix, tol=15):
                is_red = True
            if pixel_is_equal(sent4, pix, tol=15):
                is_red = True
            if pixel_is_equal(sent5, pix, tol=15):
                is_red = True
