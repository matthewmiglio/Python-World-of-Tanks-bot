...
def collect_manageable_exp_from_main():
    ...
    while n!=5:
        ...
        if details_button_coords is not None:
            ...
            pydirectinput.click(details_button_coords[0],details_button_coords[1],clicks=2,interval=0.2)
...
def scroll_up_in_details():
    ...
    pydirectinput.mouseDown(x=button_coords[0], y=button_coords[1], button='left', duration=None)
    pydirectinput.moveTo(x=destination_coords[0],y=destination_coords[1],duration=1)
    pydirectinput.mouseDown(x=destination_coords[0], y=destination_coords[1], button='left', duration=None)
    pydirectinput.click(x=destination_coords[0], y=destination_coords[1],clicks=1)
...
def find_details_button_in_details():
    ...
    #loop through every pixel in range adding every coord that is EQ to sentinel
    while y_coord<1025:
...
def verify_clusters_for_details(coord_list):
    ...
    #for each coord, look at the coords around it
        ...
        #compare the coord in question to each other coord and count the number of similar coords.
